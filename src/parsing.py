...
_MATH_STRICT_RE = re.compile(
    r"(\b[A-Za-zА-Яа-я]\s*[=+*/^]\s*[-+]?\d|\d+(?:[.,]\d+)?\s*[=+*/^]\s*[-+]?\d|[∑∫√∞≈≠≤≥±αβγδεζηθλμπρσφψω])",
    re.IGNORECASE,
)
_LATEX_INLINE_RE = re.compile(r"\$(?!\$)([^$]{1,500})\$")
_LATEX_BLOCK_RE = re.compile(r"\$\$(.+?)\$\$", re.DOTALL)
_ARTIFACT_NAME_RE = re.compile(r"^(chunks|docstore|bm25).*\.(txt|jsonl)$", re.IGNORECASE)
...
def _detect_formulas_in_text(text: str) -> List[Dict[str, Any]]:
    ...
    for line in text.split("\n"):
        ...
        if re.match(r"^\s*(рисунок|рис\.|таблица|схема)\s+", line, re.IGNORECASE):
            continue
        if _MATH_STRICT_RE.search(line):
...
def parse_pdf(file_path: Path, model_hint: str = "") -> List[DocumentElement]:
    ...
    full_text_sample = "\n".join(doc[i].get_text("text") for i in range(min(len(doc), 10)))
    ...
    try:
        for page_num in range(len(doc)):
                ...
                for heading in headings[:3]:
                    elements.append(
                        DocumentElement(
                            ...
                            source=filename,
                            page=page_num + 1,
                            model=_element_model(heading, model),
                            heading=heading,
                        )
                    )
                ...
                if table_text:
                    elements.append(
                        DocumentElement(
                            ...
                            source=filename,
                            page=page_num + 1,
                            model=_element_model(table_text, model),
                            heading=current_heading,
                            metadata={
            ...
            cleaned = normalize_ocr_text(page_text)
            if len(cleaned) >= OCR_MIN_TEXT_CHARS:
                elements.append(
                    DocumentElement(
                        ...
                        source=filename,
                        page=page_num + 1,
                        model=_element_model(cleaned, model),
                        heading=current_heading,
                    )
                )
            gc.collect()
...
def parse_text_file(file_path: Path, model_hint: str = "") -> List[DocumentElement]:
    if _ARTIFACT_NAME_RE.match(file_path.name) or looks_like_jsonl_artifact(file_path):
